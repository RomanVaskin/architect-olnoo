...
export default async function SelectedConceptPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  const selected = project.concepts.find((concept) => concept.id === project.selectedConceptId);

  if (!selected) {
    return (
      <EmptyState
        title="Основная концепция ещё не выбрана"
        description="Откройте Concepts и отметьте один из вариантов как основной для дальнейшей доработки."
      />
    );
  }
