...
/** Only the fields ProjectCard renders — deliberately narrower than Project so a /projects list summary (which has no sourceFiles/concepts/etc. loaded) can be passed here without faking empty arrays. */
  ...
  return (
    <Link href={`/projects/${project.id}`} className="group block">
      <Card className="overflow-hidden transition-colors group-hover:border-ink/20">
        <ProjectThumbnail seed={project.id} imageUrl={project.coverImage} className="h-36 w-full" />
