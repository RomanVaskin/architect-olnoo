...
- [x] Добавить чтение серверных проектов как основной источник данных вместо IndexedDB
- [x] Заменить demo-данные на дашборде `/` реальными облачными/локальными агрегатами (`src/lib/server/dashboard-repository.ts`, `GET /api/dashboard`)
- [x] Реализовать серверный пайплайн генерации/исправления концепций для облачных проектов и восстанавливаемое состояние платной попытки (`specs/cloud-generation-pipeline.md`)
